...
export class G3ZKPStorageEngine extends EventEmitter {
    ...
    async storeMessage(message) {
        ...
        let data = message;
        if (this.encryption) {
            const encrypted = this.encryption.encryptBytes(new TextEncoder().encode(JSON.stringify(message)));
            data = {
    ...
    async getMessage(messageId) {
        ...
        try {
            ...
            if (this.encryption && data.encrypted) {
                const decrypted = this.encryption.decryptBytes(new Uint8Array(data.encrypted));
    ...
    async updateConversationIndex(message) {
...
//# sourceMappingURL=storage-engine.browser.js.map